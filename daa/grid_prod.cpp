...
long long right(int grid[][20],int i,int j)
{
	if(j+3>19)
	return 0;
	else
...
long long down(int grid[][20],int i,int j)
{
	if(i+3>19)
	return 0;
	else 
...
long long max(long long ans[],int num)
{
	long long max=ans[0];
	for(int i=0;i<num;i++)
	{
...
long long diagonal(int grid[][20],int i,int j)
{
	long long ne,nw,se,sw,x,ans[2];
	/*if(i-3<0 || j+3>19)
	ne=0;
	else
...
{
	int i,j;
	long long l,r,u,d,dg,maxprod=0,x,ans[3];
	int grid[20][20]={
