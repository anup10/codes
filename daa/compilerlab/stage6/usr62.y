%{
	...
	struct nodeTypeEx*id(struct symbol_table *entry,int i);
...
%%
	...
	stmnt:
		...
		|READ '(' VAR '[' VAR ']' ')' DELIM '\n'		{temp=lookup($5->name);
									...
									{g_index=*(temp->binding);
									temp=lookup($3->name);
									if(!temp)
									printf("varialbe %s not declared\n",$3->name);
									else
									$$=opr(READ,1,id($3,g_index));
									}}
...
struct nodeTypeEx *id(struct symbol_table *index,int ind)
{
	struct nodeTypeEx *p;
...
int ex(struct nodeTypeEx *p)
{
	...
	switch(p->type)
	{
		...
		case idType:				{temp=lookup(p->id.entry->name);
							...
							{printf("%d\n id.it",p->id.it);return (*((temp->binding)+(p->id.it)));}}
		...
		case oprType:
		{
			switch(p->opr.oper)
			{
				...
				case '=':		{temp=lookup(p->opr.pr[0]->id.entry->name);
							*((temp->binding)+(p->opr.pr[0]->id.it))=ex(p->opr.pr[1]);
				...
				case READ:		{
							...
							scanf("%d",((temp->binding)+(p->opr.pr[0]->id.it)));
							return 0;}
